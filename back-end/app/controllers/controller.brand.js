...
class BrandController {
  ...
  async adminGetList(req, res) {
    try {
      ...
      const brands = data.slice().reverse()
  ...
  async adminDeleteOne(req, res) {
    const { id } = req.params;
    try {
      const products = await productModel.findOne({ brand: id });
      if (products) {
        const errorMessage =
          'Không thể xóa thương hiệu với các sản phẩm liên quan.';
        return res
          .status(500)
          .send(
            `<script>alert('${errorMessage}'); window.location.href='/brand-manager/list';</script>`
          );
      }else{
        await brandModel.deleteOne({ _id: id });
        res.redirect('/brand-manager/list');
